...
- **입력 유형 불일치 경고 추가** (2026-09-10, App5/14 버그 수정 직후 "다른 앱들도 이런 실사용 버그 있는지 확인해달라"는 요청으로 App 3/7/12/26 전체 감사 중 발견 — 4개 입력 유형(포트스캔/설정파일/코드/메모리덤프)이 서로 완전히 다른 분석기로 매핑돼 있어, 유형을 잘못 선택하면 경고 없이 대부분의 탐지가 누락되는 실제 버그를 발견함): vsftpd 백도어+Telnet+RDP가 담긴 nmap 결과를 "코드"로 잘못 선택해 분석하면 3건 중 2건이 사라지는 것을 실측으로 확인(`vuln_offline_engine.py`에서 직접 함수 호출로 재현). App5의 `_detect_type_mismatch` 패턴을 그대로 적용 — `_INPUT_TYPE_SIGNATURES`(유형별 특징 정규식: portscan=nmap/PORT STATE SERVICE, config=server{/Listen/PermitRootLogin, code=def/import/SELECT FROM/<script, memory=EPROCESS/pslist/PPID) 신규, 선택한 유형의 신호가 없는데 다른 유형의 신호가 있으면 요약 맨 앞에 경고. `vulnerability_service.py`의 SYSTEM_PROMPT에도 같은 불일치 감지 지시 추가(AI 모드 방어), `VulnerabilityScanner.jsx`에 제출 전 실시간 경고 박스+원클릭 유형 전환 버튼 추가. curl로 재현 시나리오(3건→1건 확인) 및 정상 매칭 시 오탐 없음 확인, `npm run build` 성공.
...
- 체크리스트 + 진행률 표시
- AI 채팅으로 추가 질문 가능
- **인시던트 유형↔상황 설명 불일치 경고 추가** (2026-09-10, 사용자가 "피싱 공격" 카드를 선택한 채 상황 설명에 "랜섬웨어 감염"을 입력한 스크린샷 — 대응 계획이 유형(카드)만으로 결정되고 상황 설명은 요약에 참고용으로만 붙어, 완전히 무관한 체크리스트가 경고 없이 그대로 나가는 실제 문제였음): 유형별 키워드 세트(`_TYPE_KEYWORDS` — 랜섬웨어="랜섬웨어/몸값/암호화", 피싱="피싱/스미싱" 등 6종)를 정의해 3단으로 방어 — ① `incident_offline_engine.py`가 상황 설명에서 선택 유형이 아닌 **다른** 유형의 키워드를 감지하면 "⚠️ 선택하신 사고 유형은 'OO'인데 상황 설명은 'XX'를 가리키는 것처럼 보입니다" 경고를 요약 앞부분에 자동 삽입 ② `incident_service.py`의 클라우드/로컬 AI 시스템 프롬프트(`PLAN_PROMPT`)에도 같은 불일치를 감지하면 요약 서두에 명시하도록 지시 추가(AI 모드 방어) ③ 프론트(`IncidentResponse.jsx`)는 "대응 계획 생성" 버튼을 누르기 전, 상황 설명 타이핑 즉시 동일 키워드 로직(JS로 중복 구현)으로 노란 경고 박스와 [유형을 'OO'(으)로 변경] 원클릭 버튼을 띄워 AI 모드와 무관하게 즉시 알려줌. curl로 정확히 스크린샷 상황(피싱 선택+랜섬웨어 설명)을 재현해 경고 문구가 요약에 포함되는 것, 유형·설명이 일치할 땐 오탐 없는 것까지 확인, `npm run build` 성공.
...
- **분석 유형 불일치 경고 추가** (2026-09-10, App3와 같은 세션의 실사용 버그 전수 감사 — App3 섹션 참고): 프로세스 인젝션+C2+인코딩된 PowerShell이 담긴 악성코드 설명을 "포렌식"으로 잘못 선택해 분석하면 MITRE 기법이 3건→0건, threat_level이 HIGH→LOW로 떨어지는(실제로는 위험한데 안전하다고 오인하게 만드는) 것을 실측으로 확인 — App3보다 더 위험한 사례(거짓 안전 신호). `threat_offline_engine.py`에 `_ANALYSIS_TYPE_SIGNATURES`(malware=악성코드/트로이/C2/키로깅, forensics=EventID/Prefetch/레지스트리 키, memory=pslist/pstree/volatility/malfind, threat_intel=IOC/TTP/APT/campaign) 신규, App3와 동일한 감지 로직 적용. `threat_analysis_service.py`는 유형별로 프롬프트 4개가 분리돼 있어 공용 `_TYPE_MISMATCH_NOTE` 문자열을 프롬프트 조회 시점에 덧붙이는 방식으로 4곳에 중복 작성하지 않음. `ThreatAnalysis.jsx`에 App3와 동일한 제출 전 경고 박스 추가. curl로 재현(MITRE 3→0, HIGH→LOW) 및 정상 매칭 시 오탐 없음 확인, `npm run build` 성공.
...
- **입력 유형 불일치 경고 추가** (2026-09-10, App3와 같은 세션의 실사용 버그 전수 감사 — App3 섹션 참고): 내부 URL+Stripe 키가 담긴 시스템 프롬프트를 "도구 정의"로 잘못 선택해 분석하면 3건 중 2건이 사라지는 것을 실측으로 확인. `model_audit_offline_engine.py`에 `_INPUT_TYPE_SIGNATURES`(system_prompt=you are/당신은/역할, config=rate_limit/max_tokens/api_key/"model", tools="parameters"/"function"/도구명 패턴) 신규 — JSON 키 유무로 config/tools를 구분하고 자연어 프롬프트 여부로 system_prompt를 구분하는 구조적 신호 사용. `model_audit_service.py` SYSTEM_PROMPT와 `ModelAudit.jsx`에 App3/7과 동일한 패턴 적용. curl로 재현(3건→1건) 및 config/tools 정상 매칭 시 오탐 없음 확인, `npm run build` 성공.
...
- **난이도 선택이 결과에 실제로 반영되지 않던 버그 발견·수정 + 난이도 의미/활용법 설명 추가 + 고급 난이도 정교화 + 조직 컨텍스트 반영 범위 설명 + 조직명 추출 폴백 버그 수정** (2026-09-10, 이어지는 4개 후속 요청): 사용자가 초급/고급으로 각각 생성해도 이메일 내용이 완전히 동일한 것을 스크린샷으로 지적 — 실제로 `mock_phishing_sim.py`/`phishing_sim_offline_engine.py`가 `difficulty` 값을 응답 필드에만 저장하고 실제 템플릿 선택에는 전혀 쓰지 않던(시나리오당 템플릿 1개뿐) 진짜 버그였음(클라우드 AI 호출 시에만 프롬프트로 반영됐는데 이 세션 내내 크레딧 소진으로 계속 오프라인 폴백 중이라 드러난 것).
  - `MOCK_SCENARIOS`를 시나리오×난이도(6×3=18개) 큐레이션 템플릿으로 재구성, `get_scenario_variant(scenario_type, difficulty)`로 조회하도록 mock/offline 엔진 통일 — 난이도별로 도메인의 부자연스러움·URL 노출 여부·긴급성 문구·위험 신호 개수가 실제로 다르게 작성됨
  - **"난이도" 선택에도 "의미·활용법" 설명 박스 추가** (App14 SCENARIO_MEANINGS와 같은 패턴): `DIFFICULTY_MEANINGS` 신규 — 초급(신입사원 대상 첫 캠페인)/중급(정기 훈련)/고급(경영진 등 고위험군, 성숙한 조직 재점검용) 각각 의미+활용법을 한 문단으로, `GET /api/phishing-sim/scenarios`의 `difficulties[].meaning`으로 노출
  - **"고급이 초급과 비슷하다"는 재지적에 6개 시나리오 advanced 티어 전면 정교화**: "콤보스쿼팅(combosquatting)" 기법 도입 — 실제 사내 도메인(`acme-corp.example`)을 통째로 포함한 채 뒤에 단어만 붙인 도메인(`acme-corp-support.example` 등)으로 급하게 보면 진짜처럼 보이게 함 + 시나리오별 정교화 요소(가짜 로그인 기록/티켓번호, "복리후생 개편" 같은 그럴듯한 사유, 공격자가 통제하는 개인 휴대전화를 "대체 확인 채널"로 제시하는 실제 BEC 수법, 파일 메타데이터·기기/위치/시각을 표로 제시하는 등 실제 시스템 로그 흉내) — red_flags에 기법 이름과 설명 포함해 교육 효과도 보강
  - **"조직 컨텍스트가 뭘 반영하는지" 질문에 입력창 위 안내 박스 신규**: AI 모드별로 반영 정도가 다르다는 점(외부 AI API/로컬 LLM=전체 맥락 반영, 오프라인=조직명만 추출해 ACME 치환, Mock=전혀 반영 안 됨)을 명시
  - **"라이나생명 회사 넣어도 안 바뀜" 실사용 버그 발견·수정**: 조직명 추출 정규식(`_ORG_NAME_RE`)이 "주식회사/㈜/Corp/Inc/스타트업/회사" 같은 법인 접미사가 붙어 있을 때만 매칭돼, 접미사 없이 이름만 단독 입력하면("라이나생명") 전혀 인식하지 못하던 실제 결함 — 접미사 매칭 실패 시 콤마/개행으로 구분된 첫 구간이 공백·문장부호 없는 순수 이름 형태면(문장 종결 어미로 끝나지 않는 경우만) 그 자체를 조직명으로 쓰는 폴백(`_extract_org_name`) 추가. red_flags의 signal/explanation 안에 본문을 그대로 인용한 항목("'ACME 직원님!!'처럼...")이 치환 대상에서 빠져있던 것도 함께 수정
  - **검증 중 겪은 이 세션 고유의 함정**: 백엔드가 계속 옛 코드로 응답해 원인 조사 중, 테스트 응답을 `backend/` 디렉터리 안에 파일로 저장했다 지웠다 한 것이 `uvicorn --reload`의 파일 감시에 걸려 워커가 반복 재시작되면서 이전 워커가 고아 프로세스(Windows `multiprocessing.spawn` 자식)로 남아 포트를 계속 점유하는 것을 발견 — 이후 테스트 산출물은 스크래치패드에만 저장하도록 전환. `netstat -ano`로 포트 소유 PID 확인 → `Get-CimInstance Win32_Process`로 관련 python.exe/멀티프로세싱 자식까지 전부 찾아 죽인 뒤 단일 인스턴스로 재기동하는 것으로 해결
  - curl로 18개 시나리오×난이도 조합 전부 서로 다른 내용 확인, 조직명 치환(접미사 있음/없음 양쪽) 확인, `npm run build` 성공
...
- **평가 유형 불일치 경고 추가** (2026-09-10, App3와 같은 세션의 실사용 버그 전수 감사 — App3 섹션 참고, 이 앱이 4번째이자 마지막 발견): 이 앱은 대부분의 검사가 "특정 키워드가 없으면 미비로 표시"하는 방식이라(App3/7/12처럼 탐지가 0건이 되는 게 아니라) 유형을 잘못 고르면 findings 개수는 비슷하게 나오지만 **내용이 완전히 엉뚱해지는** 더 교묘한 형태로 나타남을 실측으로 확인 — SSH root 로그인 허용+Telnet+0.0.0.0/0 노출이 담긴 CII 텍스트를 "AI 보안"으로 잘못 선택해 분석하면 "학습 데이터 검증 언급 없음" 같은 완전히 무관한 항목 4건이 나오고 실제 CRITICAL 노출은 전혀 언급되지 않으며 위험도가 MEDIUM으로 낮게 나옴(findings가 0건인 경우보다 오히려 알아채기 어려움). `kese_kit_offline_engine.py`에 `_ASSESSMENT_TYPE_SIGNATURES`(cii=기반시설/PermitRootLogin/sshd_config, ai_security=AI 모델/학습 데이터/LLM, robot_security=로봇/액추에이터/RTOS, space_security=위성/지상국/궤도, secure_coding=App3 vuln_offline_engine의 SQLi/XSS/eval/약한해시 정규식 객체를 그대로 재사용(문자열만 뽑아 재컴파일하지 않고 원본 객체 재사용 — 플래그 소실 방지), zero_trust=제로트러스트/마이크로세그먼트, supply_chain=SBOM/공급망) 신규. `kese_kit_service.py` SYSTEM_PROMPT와 `KeseKit.jsx`에 App3/7/12와 동일한 패턴 적용(프론트 `ASSESSMENT_TYPE_SIGNATURES`는 secure_coding 항목만 vuln_offline_engine 원본 정규식 대신 단순화된 JS 등가 패턴 사용). curl로 재현(findings 4건 유지되지만 내용이 CII→AI보안 무관 항목으로 바뀌고 위험도 CRITICAL→MEDIUM으로 왜곡) 및 secure_coding/zero_trust 정상 매칭 시 오탐 없음 확인, `npm run build` 성공.

### App 27: 장비 관리 & 자동 점검 (SIEM 연동) `/devices`
"SIEM 장비들이 방화벽/스위치 등에 API 호출해 자동으로 정보를 수집하는 것처럼, 이 프로그램도 장비를 등록해두면 자동으로 정보를 수집·분석하고 문제 발생 시 알림을 주는 방향으로 발전시켜달라"는 사용자 요청으로 신설. 지금까지 App 16(방화벽 감사)·18(IAM 감사)·23(실시간 모니터링)·25(증거 수집)이 전부 "사람이 텍스트를 붙여넣거나 버튼을 눌러야" 실행되던 것과 달리, 이 앱은 **장비를 한 번 등록해두면 그 뒤로는 사람 개입 없이 지정한 주기마다 자동으로 접속→수집→분석→(문제 시)알림까지** 이어진다 — 이 프로젝트 최초의 "등록형 자동화" 앱.
- **새 분석 로직을 만들지 않고 기존 부품만 조합**: 수집은 App 23(`attack_monitor_service`의 로컬/WinRM PowerShell 실행)·App 25(`forensics_collection_service`의 SSH(paramiko)·클라우드 CLI 서브프로세스 실행)를 그대로 재사용, 분석은 App 16(`firewall_audit_service.analyze_firewall`)·App 18(`iam_audit_service.analyze_iam`)·App 1(`claude_service.analyze_logs`)을 그대로 재사용. 이 앱이 새로 만든 것은 "장비 유형별로 어떤 명령을 보내 어떤 분석기의 어떤 source_type으로 넘길지" 매핑(`backend/services/device_collector.py`)과, 그걸 주기적으로 반복 실행하는 스케줄러뿐임
- **장비 유형 6종**: `network_device`(Cisco IOS/Fortinet/Palo Alto/Juniper — SSH로 `show running-config` 등 단일 조회 명령 실행, App 16의 router_switch/other 플랫폼으로 감사), `linux_host`(SSH — `iptables -L`/`nft list ruleset` 또는 인증 로그 tail), `windows_host`(로컬 PowerShell 또는 WinRM — netsh 방화벽 규칙 또는 App 23의 `collect_real_signals` 실시간 신호), `aws_account`/`azure_subscription`/`gcp_project`(이 백엔드 호스트에 이미 설치·인증된 CLI를 그대로 실행 — 원격 접속이 아니라 "어떤 자격증명으로"가 중요하다는 App 25의 설계를 그대로 따름)
- **분석 방식 3종**(장비 유형에 따라 선택지가 달라짐 — `device_store.ANALYZERS_BY_DEVICE_TYPE`): `firewall_audit`(방화벽/네트워크 정책 감사, 모든 유형에서 선택 가능)·`iam_audit`(IAM 권한 감사, 클라우드 계정 전용)·`log_analysis`(보안 로그/신호 분석, Linux/Windows 호스트 전용 — App 1 대시보드와 완전히 동일한 파이프라인·판정 기준 재사용)
- **비밀번호 암호화 저장**: 스케줄러가 사람 개입 없이 주기적으로 재접속해야 해서(App 23/25의 "저장하지 않고 매 요청 전달만 함" 원칙을 그대로 쓸 수 없음) 불가피하게 저장이 필요 — 평문 대신 대칭키(Fernet, `cryptography` 패키지 — paramiko의 기존 전이 의존성이라 새 설치 없이 바로 사용 가능했음)로 암호화해 `backend/data/device_secret.key`(history.db와 동일하게 gitignore 대상)에 보관. 조회 API(`GET /api/devices`, `/{id}`) 응답에는 암호문 필드 자체를 아예 빼고 `has_secret: bool`만 알려줘 화면·API 어디에도 값이 다시 노출되지 않음(`device_store.to_public()`)
- **스케줄러**(`backend/services/device_scheduler.py`): FastAPI startup 이벤트에서 시작하는 asyncio 백그라운드 루프가 20초마다 `enabled=1`이고 `next_run_at`이 지난 장비를 찾아 수집·분석을 실행 — SSH/서브프로세스/PowerShell 같은 블로킹 호출은 이 프로젝트 전체에서 반복된 패턴대로 `run_in_executor`로 스레드에 위임해 다른 API 요청을 막지 않음. 수동([지금 수집] 버튼)과 자동(주기 실행)이 `run_device_scan()` 하나를 공유 — 별도 경로를 만들지 않음
- **알림·통합 리스크 대시보드 자동 편입**: 결과를 `notify.APP_LABELS`에 새로 추가한 `device_monitor`("장비 관리 자동 점검") 앱 이름으로 저장해, CRITICAL 판정 시 기존 Slack/이메일/n8n 알림이 그대로 발생하고 App 22 통합 리스크 대시보드에도 별도 코드 없이 자동 집계됨(APP_LABELS 순회 설계 덕분). 다만 **App 22 프론트의 `APP_ROUTES` 매핑은 하드코딩이라 자동 편입되지 않는다는 점이 App 22/26 섹션에 이미 경고돼 있었던 대로** 이번에도 직접 추가함(안 했다면 "앱별 현황"에서 이 앱 CRITICAL 행 클릭 시 홈으로 이동하는 회귀가 재발했을 것) — 새 탐지형 앱을 추가할 때마다 반복되는 체크리스트 항목으로 재확인
- **CSV/Excel 일괄 등록** (같은 세션 후속 요청 — "개별 등록뿐 아니라 CSV/Excel 파일로도 여러 장비를 한 번에 등록하고, 예시 파일도 다운로드할 수 있게 해달라"): `POST /api/devices/bulk-upload`(`backend/services/device_bulk_import.py`)가 CSV(`csv` 표준 라이브러리) 또는 Excel(`openpyxl`, 이미 설치돼 있는 의존성 재사용) 파일을 파싱해 행 단위로 등록한다. 개별 등록과 완전히 같은 검증(`routers/devices.py`의 `_validation_error` — 기존에 `_validate`가 바로 `HTTPException`을 던지던 것을 값을 반환하도록 리팩터링해 단건/일괄 양쪽에서 재사용)을 거치되, **한 행이 잘못돼도 나머지 정상 행은 계속 등록**되고 실패한 행만 번호·이름·사유와 함께 오류 목록에 담겨 돌아옴(전부 롤백하지 않음 — 대량 등록 중 일부 오타 때문에 전체가 막히는 것을 방지). 업로드한 파일 내용(비밀번호 포함 가능)은 파싱에만 쓰이고 메모리 밖으로 저장되지 않음(App 19 시크릿 스캐너의 "원본 미저장" 원칙을 여기에도 적용)
  - 컬럼: `name`/`device_type`/`analyzer`(필수), `vendor`/`host`/`port`/`username`/`password`/`context`/`interval_minutes`/`enabled`(선택) — 헤더 셀의 공백·대소문자·하이픈은 정규화해서 인식(`device_bulk_import._normalize_header`)
  - 예시 파일(`frontend/public/samples/devices/device-bulk-template.csv`)에 4개 장비 유형(network_device/linux_host/windows_host/aws_account)을 각각 대표하는 행을 미리 채워뒀고, 화면에 [예시 파일 다운로드] 버튼과 컬럼 설명 표를 함께 제공
  - 정상 4행 전부 등록·비밀번호 필요 유형만 `has_secret:true`로 정확히 반영되는 것, 그리고 이름 누락/잘못된 device_type/network_device의 vendor 누락/허용 안 되는 analyzer 조합/숫자로 변환 안 되는 interval_minutes 5가지 오류 케이스가 각각 정확한 한국어 메시지로 걸러지면서도 그 사이 정상 행 1건은 그대로 등록되는 것까지 curl로 end-to-end 검증
- `backend/routers/devices.py`, `backend/services/device_store.py`(SQLite `devices` 테이블 — `db.py`의 범용 JSON 블롭 히스토리와 별개로, 스케줄러가 `enabled`/`next_run_at`을 SQL로 직접 걸러야 해서 전용 컬럼을 둠)/`device_collector.py`/`device_scheduler.py`/`device_bulk_import.py`/`crypto_util.py`
- 백엔드는 CRUD(생성/조회/수정/삭제)·연결 테스트(저장 전/후)·[지금 수집](이 PC를 대상으로 한 `windows_host`+`log_analysis` 조합으로 실제 App 23 실시간 신호 수집 경로까지 end-to-end 실행 — 이 세션이 크레딧 소진 상태라 오프라인 폴백까지 함께 확인됨)·장비별 히스토리 조회·CSV 일괄 등록(정상/오류 혼합)까지 curl로 전부 검증, `device_monitor`가 App 22 대시보드 집계에 자동으로 반영되는 것도 확인. `npm run build` 성공 — 이 세션은 Chrome 확장이 연결되지 않아 실제 브라우저 렌더링(장비 카드, 폼 전환, 일괄 업로드 결과 표시 등)은 사용자 확인 필요
- **⚠️ 이 세션에서 겪은 환경 이슈(코드 버그 아님)**: `devices.py` 수정 후 `uvicorn --reload`가 "Reloading..."까지만 로그에 남기고 워커를 재시작하지 못해(App 16/22/25 섹션에 이미 여러 차례 기록된 것과 동일 계열) 포트 8000을 쥔 채 멈췄는데, 이번엔 한 단계 더 나아가 **`Get-Process`/`Get-CimInstance`/`tasklist` 세 도구 모두 그 PID를 찾지 못하는데도 `netstat`/`Get-NetTCPConnection`에는 여전히 LISTENING으로 잡히는** 새로운 형태로 나타남(기존 "좀비 소켓" 사례보다 더 심함). 원인은 리로더 자체(그 PID)가 아니라 리로더가 띄운 **자식 워커 프로세스**가 진짜로 소켓을 쥔 채 남아있었던 것 — `netstat`이 보고하는 OwningProcess가 실제로는 부정확했음. `Get-Process`로 같은 시각에 시작된 다른 python.exe(워커로 추정되는 PID)를 찾아 그것까지 종료하자 비로소 포트가 풀림. **교훈**: `--reload` 좀비 소켓이 의심되는데 netstat이 지목한 PID가 이미 존재하지 않는다면, 포기하지 말고 `Get-Process | Where ProcessName -match python`으로 그 시점에 떠 있는 다른 python 프로세스(리로더의 자식 워커)를 찾아 함께 정리할 것.

#### App 27 세션 2 — 로컬 LLM 실지 검증 + 실제 AWS 연동 + AI 에이전트 연계 4종 (2026-09-12)

"이 프로그램을 로컬 LLM으로 확인하고 싶다"는 요청으로 시작해, "AWS 계정과 연동해서 실제 장비로 검증", "AI 에이전트로 연계"까지 이어진 하루 세션. 매우 넓은 범위라 순서대로 기록한다.

- **로컬 LLM(Ollama) 실제 설치·검증**: 이 PC에 로컬 LLM이 없어 winget으로 Ollama 설치 + `llama3.2:3b`(2GB, CPU 전용 환경에 적합한 소형 모델로 사용자와 상의해 선택) 다운로드. 첫 다운로드는 DNS 조회 실패로 중단됐으나 재시도로 성공(일시적 네트워크 문제로 판단). `backend/.env`에 `LOCAL_LLM_BASE_URL=http://localhost:11434/v1`/`LOCAL_LLM_MODEL=llama3.2:3b` 추가 후 `POST /api/mode/override {"mode":"local"}`로 명시 전환(외부 API 키가 설정돼 있으면 자동 감지가 계속 cloud를 우선하므로 override 필요).
  - **품질 한계를 실측으로 확인**: 방화벽 감사기·취약점 스캐너·AWS 보안그룹 감사 등 여러 화면에서 실제 로컬 모델 응답을 검증한 결과, JSON 구조/판정 형식은 안정적으로 따르지만 **사실 관계에서 명백한 오류를 반복 생성**함을 확인 — vsftpd 2.3.4를 "Heartbleed 포함"이라고 설명(실제로는 무관한 OpenSSL 취약점과 혼동), "0.0.0.0/0을 0.0.0.0/0으로 바꾸세요"처럼 자기모순적인 권장 조치, SSH를 통째로 차단하라는 위험한 제안 등. **오프라인 규칙 기반 엔진이 오히려 더 정확한 경우도 실측**(AWS 6개 보안그룹 감사에서 로컬 LLM 대신 오프라인 엔진이 자동 폴백돼 실행됐는데, 할루시네이션 없이 CRITICAL 3건+HIGH 1건+LOW 1건을 정확히 탐지). **결론(문서화된 교훈)**: 이 프로젝트에서 로컬 소형 모델은 "AI가 완전히 없는 것보다는 낫다"는 폐쇄망 최후 수단으로는 유효하지만, CVE 매핑처럼 사실 정확도가 중요한 판단에는 실시간 CVE 조회(App 15)로 반드시 교차검증하라고 안내해야 함.
  - **AI 모드 override가 세션 도중 원인 불명으로 "offline"로 되돌아간 사례 발견** — 재현하지 않고 다시 override로 복구함. 정확한 원인은 못 찾았으나, 로컬 LLM으로 작업할 때는 중간중간 `GET /api/mode`로 `effective_mode`를 재확인하는 습관이 필요함을 기록.

- **실제 AWS 계정 연동 검증**: 사용자가 미리 만들어둔 실제 AWS 계정(`security-agent-user`, 리전 `ap-northeast-2`)이 이 PC에 AWS CLI로 이미 인증되어 있음을 확인. App 27의 `aws_account` 장비로 등록해 **실제 계정의 보안그룹을 감사** — LocalStack 샌드박스(test-range)가 아닌 진짜 AWS API 호출로 App 27이 동작함을 처음 검증.
  - **착수 전 필수 확인**: 이 IAM 사용자가 **AdministratorAccess(전체 관리자 권한)**를 가지고 있고, 계정에 **"Zero-Spend Budget"($1 한도)**이 이미 설정돼 있음을 먼저 확인 — 실제 리소스 생성은 이 신호를 "비용을 전혀 쓰고 싶지 않다"는 뜻으로 해석해 항상 무료/최소 비용 경로를 우선하고, 사용 후 즉시 정리하는 방침을 세웠다(AskUserQuestion으로 범위를 확인한 뒤 진행).
  - **AWS 내 "다수 장비" 시뮬레이션**: 보안그룹 5개(웹티어=HTTP/HTTPS 공개는 정상 허용 케이스로 의도적 포함, DB티어=MySQL 전체공개, 관리용 배스천=SSH/RDP 전체공개, 레거시 관리=Telnet/SNMP, SSH 테스트 전용=이 PC IP만 허용) 신규 생성 — 보안그룹 자체는 무료라 비용 없이 "여러 방화벽 정책을 가진 장비"를 감사 대상으로 만듦(`Project=ai-security-suite-test` 태그). AWS엔 전통적 "스위치" 개념이 없어 억지로 흉내내지 않고 보안그룹/NACL 축으로만 다양화함을 명시.
  - **EC2 인스턴스 2대(t3.micro, 프리티어) 실제 기동 + SSH 실접속 검증**: Amazon Linux 2023, SSH는 이 PC의 공인 IP로만 제한하는 전용 보안그룋 사용. **검증 후 즉시 terminate + 키 페어 삭제**로 비용이 남지 않게 함(원칙: 실제 요금이 발생할 수 있는 리소스는 검증이 끝나면 바로 정리).
  - **⚠️ 이 과정에서 발견한 진짜 기능 공백 — SSH 키 인증 미지원**: 지금까지 `network_device`/`linux_host` 장비는 비밀번호 인증만 지원했는데, AWS EC2 등 실제 클라우드 VM은 기본적으로 비밀번호 인증이 꺼져 있고 키 페어만 허용한다 — 그래서 실제 클라우드 서버엔 애초에 붙을 수 없었던 결함이었다. 수정 범위:
    - `forensics_collection_service.py`(App 25가 이미 갖고 있던 SSH 실행기)에 `_load_private_key()`(RSA/Ed25519/ECDSA 순서로 시도해 PEM 파싱) 신규, `_ssh_connect`/`check_ssh_connection`/`_run_ssh_commands`에 선택적 `pkey` 매개변수 추가(하위 호환 유지, 기존 password 인자는 그대로 둠)
    - `device_store.py`의 저장 데이터에 `auth_method`("password"|"private_key") 필드 추가 — private_key일 땐 기존 `password` 필드에 PEM 텍스트를 그대로 담아 암호화 저장(별도 필드를 새로 만들지 않고 "비밀 값 하나"라는 기존 모델을 그대로 재사용)
    - `device_collector.py`에 `_pkey_for(device)` 헬퍼 추가, network_device/linux_host 수집·연결테스트 경로 전부에서 auth_method에 따라 비밀번호/키 중 하나를 사용하도록 분기
    - `routers/devices.py`의 `DevicePayload`/`_validation_error`에 `auth_method` 검증 추가
    - 실제 EC2 인스턴스 대상으로 키 인증 연결 테스트 성공 + `linux_host` 장비로 등록해 실제 `iptables`/`nftables` 규칙을 SSH로 가져와 감사까지 end-to-end 검증 완료(로컬 LLM이 "management 포트에 broad source" 등을 탐지했으나 이번에도 일부 권장 조치 문구가 부정확했음 — 위 로컬 LLM 품질 한계와 같은 패턴)

- **⚠️ 이 세션에서 발견해 고친 진짜 버그 — 이벤트 루프 블로킹**: 여러 장비를 동시에 스캔하는 중 `GET /`조차 응답하지 않는 전체 행 걸림 현상을 실제로 겪음. 원인: `firewall_audit_service.analyze_firewall()`/`iam_audit_service.analyze_iam()`이 cloud/local 모드일 때 동기(블로킹) 네트워크 호출인 `_real_analyze()`를 `run_in_executor` 없이 그대로 `await` 위치에서 호출하고 있었음 — `claude_service.analyze_logs()`(App 1)는 이미 이 패턴을 따르고 있었는데 두 서비스만 누락돼 있었던 것. 단일 요청·빠른 클라우드 API 응답 환경에서는 거의 드러나지 않다가, App 27 스케줄러가 **여러 장비의 로컬 LLM 호출(수십~120초)을 연달아 발생**시키면서 처음으로 명확히 드러남 — "동시에 여러 장비를 스캔하면 앱 전체가 멈춘다"는 실사용 버그였다. 두 파일 모두 `asyncio.get_event_loop().run_in_executor(None, _real_analyze, ...)`로 수정해 다른 API 요청이 막히지 않음을 동시 스캔 2건 실행 중 `/api/mode`·`/api/devices` 응답으로 검증. **다른 13개 서비스(App 2/3/5/7/11/12/14/16/20/24/26 등)도 동일한 `_real_analyze()` 직접 호출 패턴을 쓰고 있어 잠재적으로 같은 문제를 안고 있을 가능성이 높음** — 이번엔 시간 관계상 firewall_audit/iam_audit 두 곳만 수정, 나머지는 향후 전수 점검 대상으로 남겨둠.
  - **부수적으로 겪은 함정**: `local_llm_client.py`의 httpx 타임아웃이 120초로 설정돼 있는데 curl 테스트 타임아웃을 60~90초로 짧게 잡아 "멈췄다"고 오인한 적이 있었음 — 보안그룹이 늘어나 프롬프트가 커지면 CPU 3B 모델은 실제로 2분 가까이 걸릴 수 있으므로, 이 프로젝트에서 로컬 LLM을 테스트할 땐 항상 120초 이상의 여유를 두고 판단할 것.

- **AI 에이전트 연계 — 4가지 방향** ("MCP 도구화(추천)/SOAR 자동 대응/정기 점검 보고서/Bedrock Agent" 중 전부 선택):

  1. **MCP 도구화 (완료)** — `mcp-server/server.py`: 이 앱을 새로 분석하지 않고, 이미 떠 있는 백엔드 REST API를 그대로 호출하는 얇은 래퍼로 MCP 서버 구현(`mcp` SDK 2.x, `MCPServer` — v1의 `FastMCP`는 이 버전에서 이름이 바뀜). 장비 등록/조회/즉시수집/이력조회/삭제, 알림 조회, 통합 리스크 현황 조회, 방화벽 규칙 즉석 감사, IoC 분석, CVE 조회까지 11개 도구 제공. Claude Desktop/Code 설정 예시와 사용법은 `mcp-server/README.md`.
     - **⚠️ 설치 중 실제로 백엔드를 깨뜨릴 뻔한 의존성 충돌 발견**: `pip install "mcp[cli]"`가 `starlette`를 FastAPI 0.115.0이 요구하는 범위(`<0.39.0`)를 벗어난 최신 버전(1.6.0)으로 강제 업그레이드해, 그대로 뒀으면 메인 백엔드가 깨질 뻔했음 — 즉시 `pip install "starlette>=0.37.2,<0.39.0"`로 되돌려 백엔드 정상 작동 확인. **교훈 + 대응**: `mcp-server/README.md`에 "반드시 별도 가상환경에 설치할 것" 경고를 최상단에 명시 — 백엔드와 같은 Python 환경에 `mcp[cli]`를 설치하면 안 됨.
     - 설치 중 겪은 또 다른 환경 문제: 이 세션 내내 반복된 "`uvicorn --reload` 좀비 소켓" 패턴이 이번엔 한 단계 더 진화한 형태로 재발(위 App 27 세션 2 섹션 참고) — pip이 uvicorn 패키지 파일을 교체하려다 실행 중인 프로세스에 잠겨 실패, 관련 프로세스를 모두 정리한 뒤 설치 성공.
     - `list_devices`/`get_risk_overview`/`lookup_cve` 도구 함수를 직접 호출해 실제 백엔드 데이터가 정상적으로 오는 것 확인.

  2. **정기 점검 보고서 에이전트 (완료)** — `scripts/daily-security-report.ps1` + Windows 작업 스케줄러("AI Security Suite Daily Report", 매일 09:00). `schedule` 스킬(클라우드 예약 에이전트)로 먼저 시도했으나 **클라우드 에이전트는 격리된 샌드박스라 이 PC의 `localhost:8000`에 접근할 수 없어 이 용도에 원천적으로 맞지 않음**을 확인하고 즉시 로컬 스케줄로 전환 — 클라우드/로컬 에이전트를 고를 때 "로컬 리소스 접근 필요 여부"가 결정적 기준임을 기록.
     - 동작: PowerShell이 결정론적으로 `GET /api/dashboard/overview`+`GET /api/alerts`를 조회 → 이 PC에 로그인된 **Claude Code CLI(구독, API 크레딧 아님)**를 헤드리스(`-p --tools ""`)로 호출해 한국어 요약만 맡김 — Claude에 도구 접근을 주지 않고(App 1의 `claude_cli_client.py`와 동일한 안전 원칙) 무인 실행 시 도구 승인 프롬프트가 막히는 것을 원천 차단.
     - **⚠️ 실제로 겪은 PowerShell 함정 2건**: ① `Get-Command claude`가 resolve하는 `claude.ps1` 래퍼는 파이프로 넘긴 stdin을 내부 node 프로세스에 제대로 전달하지 못해("Input must be provided either through stdin or as a prompt argument") 조용히 실패 — Bash의 `echo | claude`는 되는데 PowerShell의 `"..." | & claude.ps1`은 안 되는, 이 프로젝트에서 반복돼 온 "셸마다 동작이 다르다" 유형의 새 사례. `cmd.exe`의 진짜 파일 리다이렉션(`<`)으로 `claude.cmd`(APPDATA\npm 경로)를 호출하는 방식으로 우회. ② `$summary = cmd.exe /c "..."`처럼 외부 프로세스 출력을 PowerShell 변수로 캡처하면 한글이 깨짐(PowerShell이 콘솔 코드페이지로 잘못 디코딩) — `cmd.exe` 자신의 `>` 파일 리다이렉션으로 stdout 바이트를 파일에 직접 쓰게 해 PowerShell의 문자열 캡처 단계 자체를 건너뛰어 해결.
     - 실제 리스크 데이터로 여러 번 실행해 정확한 한국어 요약(우선순위 판단, 근거 포함)이 나오는 것 확인 — 심지어 n8n이 꺼져 있어 알림 전송이 실패하고 있다는 부수적 문제까지 스스로 짚어냄. Windows 작업 스케줄러로 트리거했을 때도(로그인 세션 컨텍스트) 동일하게 정상 동작 확인. 보고서는 `backend/data/daily_reports/`(gitignore 대상)에 타임스탬프 파일로 저장.

  3. **SOAR 자동 대응 제안 워크플로우 (n8n, 구축 완료·최종 라이브 검증은 보류)** — 사용자가 `docker compose up`으로 n8n을 다시 띄운 뒤 진행. 기존 "AI Security Suite - Critical Alerts → Slack" 폴링 워크플로우(5분마다 `/api/alerts` 조회 → 신규만 필터 → Slack)를 **복제**해 "AI Security Suite - SOAR Remediation Proposal"로 신설, Filter와 Slack 사이에 로컬 Ollama(`http://host.docker.internal:11434/v1/chat/completions`) 호출 노드를 추가해 각 CRITICAL 알림에 대한 구체적 수정 제안을 Slack 메시지에 덧붙임. **자동 실행이 아니라 "제안"임을 시스템 프롬프트와 Slack 메시지 양쪽에 명시**하고, 실제 AWS 자동 조치(예: 보안그룹 규칙 자동 삭제)는 이번엔 구현하지 않음 — AdministratorAccess 권한의 실계정에 자동 쓰기 액션을 붙이는 것은 되돌리기 어려운 위험이라 사람이 검토 후 직접 적용하는 현재 방식을 기본값으로 유지.
     - n8n 로그인은 브라우저에 저장된 자격증명을 사용자가 직접 "Sign in" 클릭해 완료(비밀번호 자동입력 필드를 대신 제출하지 않음).
     - **⚠️ n8n HTTP Request 노드의 "JSON Body" 필드 작성법을 잘못 이해해 반복 실패 후 원리를 파악**: 이 필드는 전체를 `{{ 자바스크립트 객체 }}` 하나로 감싸는 방식이 아니라, **필드 자체가 먼저 순수 JSON 텍스트로 파싱되고, `{{ }}` 표현식은 오직 JSON 문자열 값 내부에서만 허용**됨 — `{{ JSON.stringify({...}) }}`처럼 전체를 감싸거나 따옴표 없는 JS 객체 리터럴 키(`model:` 등)를 쓰면 "Expected property name" 오류가 남(최소 재현: `{{ JSON.stringify({a:1}) }}`조차 실패). 올바른 형태: `{"model": "llama3.2:3b", "messages": [..., {"role":"user","content":"{{ 'literal ' + $json.field }}"}], ...}`처럼 큰따옴표로 감싼 정상 JSON 구조를 쓰고, 동적 값이 필요한 곳에만 문자열 내부에 `{{ }}`를 심는다. **일반화된 교훈**: n8n의 JSON 타입 파라미터는 "표현식 모드"라 해도 파서가 먼저 유효한 JSON 문법을 기대하며, JS 객체 리터럴 문법(따옴표 없는 키)과 완전한 JSON 문법(모든 키/문자열 큰따옴표)을 혼동하면 이런 오류가 남.
     - 실제 알림 데이터(app_label/severity/summary/target/recommendation 필드명)로 Filter 노드 출력을 직접 확인해 필드 매핑이 정확함을 검증, JSON 오류 수정 후 실제로 Ollama까지 요청이 도달하는 것 확인.
     - **⚠️ 최종 라이브 실행은 이 PC의 메모리 부족(여유 1.5GB/14GB)으로 Ollama가 모델을 로드하지 못해("unable to allocate CPU_REPACK buffer") 보류** — 워크플로우 로직 자체는 검증됐으나 실제 Slack 메시지 도착까지는 확인하지 못함. 다른 프로그램을 정리하거나 이 노드를 클라우드 API로 바꾸면 바로 동작할 것으로 판단. 워크플로우는 **Unpublished(비활성) 상태로 저장**해둠 — 사용자가 준비되면 직접 Publish.

  4. **AWS Bedrock Agent 연동 (보류, 다음 세션)** — 실제 배포하려면 Bedrock 콘솔에서 모델 액세스 활성화(콘솔 전용 작업)와 액션 그룹용 Lambda/OpenAPI 스키마가 필요하고, 무엇보다 **호출마다 실제 과금이 발생**해 이 계정의 "Zero-Spend Budget" 설정과 직접 충돌함 — 사용자가 비용을 감수할지 확인 후 다음 세션에서 진행하기로 함(설계만 논의, 리소스는 아직 만들지 않음).
...
- **AI 실행 모드 (cloud/local/offline/mock)** (2026-09-05, 폐쇄망 지원 롤아웃): 기존 Mock/Live 2모드를 4모드로 확장 — `cloud`(Claude API)/`local`(사내 로컬 LLM)/`offline`(네트워크 없이 동작하는 규칙 기반 실제 분석)/`mock`(기존 방식의 고정 샘플, 학습용으로 명시적 선택시에만). `backend/services/mode_manager.py`가 `ANTHROPIC_API_KEY`·`LOCAL_LLM_BASE_URL` 설정 여부와 실제 네트워크 도달 가능 여부를 함께 봐서 cloud→local→offline 순으로 자동 감지하고, NavBar의 `ModeSelector`(`GET/POST /api/mode`, `/override`)로 전 앱 공통 수동 전환도 가능(재시작에도 유지). Claude를 쓰는 16개 앱(대시보드·실시간모니터링/피싱/취약점/IoC/인시던트/위협분석/인젝션탐지/정책생성기/모델감사/피싱모의훈련생성기/방화벽·IAM·컨테이너 감사기/금융보안원 CSP평가) 전부 이 패턴으로 전환 완료 — 각 앱은 `mode_manager.get_ai_mode()`로 분기해 offline일 때 `<app>_offline_engine.py`(정규식/키워드 기반 실제 입력 분석 — 탐지형은 vuln_offline_engine.py, 생성형은 policy_offline_engine.py처럼 템플릿+키워드 커스터마이즈 패턴)로 위임하고, cloud/local 호출이 런타임에 실패하면 자동으로 offline로 폴백(`fallback_reason` 기록). App 25(포렌식 실습·분석 센터)의 '아티팩트 감사기' 탭도 같은 패턴(`forensics_audit_offline_engine.py`)이라 사실상 17개 앱째지만, 같은 앱 안의 '실습 랩'·'증거 수집 도구' 탭은 처음부터 AI를 쓰지 않아(App 25 섹션 참고) 앱 하나가 두 부류에 걸쳐 있는 이 프로젝트 최초의 사례. Claude를 원래 안 쓰던 앱(웹스캐너/Pwn Lab/Web CTF/모의해킹랩/인프라스캐너/시크릿스캐너/DNS보안/통합대시보드)은 대상 아님. App 15(CVE 조회)처럼 Claude가 아니라 외부 실시간 API에 의존하는 앱은 `mode_manager.get_external_api_mode()`라는 별도 online/offline 축을 쓰며, 로컬 캐시(write-through)+공식 데이터 피드 가져오기로 폐쇄망을 지원(App 3/15 섹션 참고). 상세 설계·발견한 버그는 App 3 섹션의 "폐쇄망(오프라인) 지원 + 로컬 LLM 연동" 참고 — 나머지 앱들도 동일 패턴이라 개별 섹션에 중복 기술하지 않음.
  - **로컬 LLM 실지 검증 완료** (2026-09-12): 이 PC에 Ollama + `llama3.2:3b`(2GB, CPU 전용) 설치해 실제로 `local` 모드 end-to-end 검증(App 27 세션 2 섹션 참고). 구조화된 JSON 출력은 안정적이나 **사실 정확도(CVE 매핑 등)에서 반복적인 할루시네이션을 실측** — 폐쇄망 최후 수단으로는 유효하나 정확도가 중요한 판단에는 App 15(CVE 조회)로 교차검증할 것을 권장하는 게 문서화된 결론.
...
파일 파싱: python-docx / pypdf / openpyxl (Word/PDF/Excel 업로드 텍스트 추출, App 27 장비 CSV/Excel 일괄 등록)
원격 수집: paramiko (App 25 '증거 수집 도구' 탭·App 27 장비 관리의 SSH 기반 Linux/macOS/네트워크 장비 수집)
암호화:   cryptography (App 27 장비 접속 비밀번호 저장 시 암호화 — paramiko의 기존 전이 의존성 재사용)
로컬 LLM: Ollama + llama3.2:3b (이 PC에 실제 설치·검증됨, OpenAI 호환 /v1/chat/completions)
에이전트 연동: mcp-server/(MCP 도구화, 별도 가상환경 필수) · Windows 작업 스케줄러(정기 보고서) · n8n(SOAR 제안 워크플로우)
...
├── mcp-server/                 ← AI Security Suite를 MCP 도구로 노출 (Claude Desktop/Code 연동, App 27 섹션 참고)
│   ├── server.py               ← FastMCP(mcp[cli] 2.x, MCPServer) — 백엔드 REST API를 감싼 도구 11종
│   ├── requirements.txt        ← 반드시 별도 가상환경에 설치할 것(README 참고 — starlette 버전 충돌로 백엔드가 깨짐)
│   └── README.md                ← Claude Desktop/Code 등록 방법, 제공 도구 목록
├── scripts/
│   └── daily-security-report.ps1 ← 정기 점검 보고서 에이전트 (Windows 작업 스케줄러, 매일 실행, App 27 섹션 참고)
├── backend/
│   ├── main.py
│   ├── requirements.txt
│   ├── routers/
...
│       ├── crypto_util.py     ← App 27 장비 접속 비밀번호 암호화(Fernet), backend/data/device_secret.key
...
| `/devices`(장비 관리) — 등록/수정/CSV·Excel 일괄 등록/목록 조회 자체 | 없음 — 서버 두 개만 켜면 바로 가능. `windows_host` 장비를 호스트를 비워둔 채(이 PC 자신) 등록하면 접속 정보 없이도 바로 자동 점검이 동작함 |
| `/devices`에 `network_device`/`linux_host`/원격 `windows_host` 등록 | 대상 장비에 SSH(또는 WinRM) 접속 가능해야 하고, 등록 시 입력한 사용자명/비밀번호로 실제 로그인 가능해야 함(`/forensics`·`/attack-monitor`의 원격 수집과 같은 요구사항) |
| `/devices`에 `aws_account`/`azure_subscription`/`gcp_project` 등록 | 이 백엔드 호스트에 해당 CLI(aws/az/gcloud)가 설치·인증되어 있어야 함 — 원격 접속이 아니라 로컬 CLI를 그대로 실행(`/forensics`의 클라우드 CLI 수집과 동일) |
...
- **2026-09-11 후속 — Ghidra 신규 설치 + Reverse crackme 재검증 진행 중**: 새 컨테이너에서 ret2win/ret2system flag 재확인 완료, fmtstr는 재빌드만 하고 flag 재확인은 다음으로 미룸. crackme는 소스+Python XOR 디코딩(`Gh1dra_Pr0!!`)까지는 확인했으나 `./crackme` 실행으로 최종 flag(`RE{gh1dra_and_x0r_ar3_fr1ends}`) 출력은 아직 미확인. 이 PC에 **Ghidra 12.1.3(공식 NSA GitHub 릴리스) + JDK 21(Eclipse Temurin, winget)을 신규 설치**함(`C:\Tools\ghidra_12.1.3_PUBLIC\ghidraRun.bat`) — crackme의 `check()` 함수를 실제로 Import→Analyze→디컴파일해서 소스/Python 결과와 정확히 일치하는 것까지 교차검증 완료. ⚠️ winget 설치 직후 같은 PowerShell 세션에서 바로 실행하면 `$env:PATH`가 설치 전 상태로 캐시돼 있어 `java`를 못 찾고 조용히 실패하니, 새 셸을 열거나 PATH를 수동 갱신할 것.
...
- **AWS Bedrock Agent 연동 — 다음 세션에서 진행** (2026-09-12, App 27 "AI 에이전트 연계" 4방향 중 마지막 하나): MCP 도구화/정기 점검 보고서 에이전트/SOAR n8n 워크플로우는 완료했고, Bedrock Agent만 보류함. 실제 배포하려면 (1) Bedrock 콘솔에서 사용할 모델(Claude 등) 액세스를 수동으로 활성화, (2) 액션 그룹용 Lambda 함수 + OpenAPI 스키마 작성(이 앱의 REST API를 액션으로 노출), (3) IAM 역할 구성이 필요함. **다음 세션 시작 시 먼저 확인할 것**: 이 계정("Zero-Spend Budget" $1 설정됨, AdministratorAccess 보유 — App 27 세션 2 섹션 참고)에 Bedrock 호출 비용(토큰당 과금, 완전 무료 불가)을 사용자가 감수하기로 했는지 재확인한 뒤 착수. 진행하기로 하면: Lambda는 기존 `device_collector.py`/`routers/devices.py`의 엔드포인트를 그대로 호출하는 얇은 래퍼로 구성(MCP 서버와 동일한 "새로 분석하지 않고 기존 API를 감싼다" 원칙 재사용 권장).
- **App 27 후속 — 다른 12개 서비스의 이벤트 루프 블로킹 잠재 버그 점검** (2026-09-12 발견): `firewall_audit_service.py`/`iam_audit_service.py`는 `_real_analyze()`를 `run_in_executor` 없이 직접 호출하고 있어 여러 장비 동시 스캔 시 앱 전체가 멈추는 실제 버그였고 이번에 수정함(App 27 세션 2 섹션 참고). **같은 패턴(`_real_analyze` 또는 동등한 동기 블로킹 호출을 async 함수에서 직접 await)을 쓰는 나머지 서비스들(App 2 phishing/App 3 vulnerability/App 5 incident/App 7 threat_analysis/App 11 policy/App 12 model_audit/App 14 phishing_sim/App 16 container_audit/App 20.../App 24 fsi_csp_audit/App 26 kese_kit 등)도 잠재적으로 동일 문제를 안고 있을 가능성이 높음** — 이번엔 시간 관계상 App 27 스케줄러가 실제로 호출하는 두 곳만 고쳤음. 전수 점검해 동일하게 `run_in_executor`로 감싸는 작업이 남아있음(특히 로컬 LLM처럼 응답이 느린 백엔드를 쓸 때 체감됨).
- **외부 시연 준비 — 진행 중, 다음 세션에서 이어서** (2026-09-10 시작, 2026-09-11 후속): ngrok 터널·AnyDesk 원격조작·Zoom 화면공유 조합으로 외부 PC 대상 시연 환경을 구성 중, 겸해서 Pwn Lab 전체 챌린지 재검증도 이어가는 중.
  - 완료: ngrok 설치+인증+`vite.config.js` allowedHosts 수정, AnyDesk 설치+무단액세스 비밀번호 설정(ID 1217880678)
  - **2026-09-11 재검증**: 새 컨테이너에서 ret2win(`PWN{r3t2w1n_st4ck_sm4sh1ng_101}`)·ret2system(가젯 주소 전부 이전과 동일 확인 — pop_rdi_ret=0x4011f8/ret=0x40101a/cmd=0x404080, `PWN{r3t2sy5tem_ropp1ng_w1th_style}`) flag까지 재확인 완료. fmtstr는 재빌드만 하고 flag 재확인 미완(`./fmtstr` 실행 전에 셸 프롬프트에 바로 포맷 문자열을 입력해 bash job-control 오류를 겪고 중단 — 프로그램을 먼저 실행한 뒤 그 입력 대기 상태에 포맷 문자열을 넣어야 함).
  - **Reverse crackme 진행 중**: 소스 직접 확인 + Python XOR 디코딩(`Gh1dra_Pr0!!`) + **Ghidra 디컴파일 3중 교차검증**까지 완료(check() 디컴파일 결과가 소스와 정확히 일치). 다만 `./crackme`에 비밀번호를 실제로 입력해 `RE{gh1dra_and_x0r_ar3_fr1ends}` flag 출력까지는 아직 미확인.
  - **이 세션에서 이 PC에 Ghidra 신규 설치 완료** — 이전엔 이 PC에 Ghidra가 없어 Reverse 챌린지를 소스/Python으로만 검증했는데, 이번에 정식 설치함: JDK 21(Eclipse Temurin, winget `EclipseAdoptium.Temurin.21.JDK`) + Ghidra 12.1.3(공식 NSA GitHub 릴리스, GitHub API로 최신 버전 확인 후 다운로드) → `C:\Tools\ghidra_12.1.3_PUBLIC\`에 압축 해제. **⚠️ 설치 직후 겪은 함정**: 새 JDK가 시스템 PATH엔 반영됐지만 이미 열려있던 PowerShell 세션의 `$env:PATH`는 설치 전 상태로 캐시돼 있어(그 안에 이미 존재하지도 않는 옛 JDK11 경로가 낡은 채로 남아있기까지 했음) `ghidraRun.bat`가 `java`를 못 찾아 **조용히** 실행 실패함 — `$env:PATH`에 새 JDK21 bin 경로를 직접 prepend한 뒤에야 정상 실행됨. **교훈**: winget 설치 직후 같은 세션에서 바로 실행하려면 PATH를 수동 갱신하거나 새 셸을 열 것.
  - 남은 것: fmtstr flag 확인, crackme 실제 실행으로 flag 확인, keygen_check·antidebug_crackme(Reverse) 및 Misc 3종(encoding-chain/zerowidth-stego/osint-clues) 재검증, 그리고 원래 목적인 실제 원격 PC AnyDesk 접속 후 Docker 컴파일 end-to-end 검증.
  - 상세는 위 "실행 방법"의 "외부(원격 PC)에서 시연하기" 참고. **재개 시 확인할 것**: ngrok 터널·백엔드·프론트엔드가 여전히 켜져 있는지 `netstat`으로 확인(재부팅됐으면 재기동 필요, ngrok URL은 재시작 시 바뀜). 이번에 설치한 Ghidra는 재부팅해도 `C:\Tools\ghidra_12.1.3_PUBLIC\ghidraRun.bat`로 새 셸에서 그대로 재실행 가능(PATH 문제 없음).
...
(그 외에는 2026-09-05 폐쇄망/AI 실행 모드 롤아웃이 대상 16개 앱 전부 완료됨. 상세는 위 "공통 기능"의 "AI 실행 모드" 항목과 App 3/15 섹션 참고. 2026-09-06 커밋 `5942131`로 `origin/master`에 푸시 완료. 2026-09-10 App5 유형-설명 불일치 경고, App14 난이도별 결과 미반영/조직명 추출 버그 수정 — 커밋 `e7f5590`으로 `origin/master`에 푸시 완료. 같은 날 후속으로 App3/7/12/26에도 동일 계열의 입력 유형 불일치 경고 추가 — 커밋 `14e474a`로 `origin/master`에 푸시 완료.)
