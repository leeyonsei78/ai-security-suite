...
- **실제 사이트 대상 사용 가능 여부 검토** (2026-09-04, 사용자 질문에 답하며 `webscan_service.py` 코드 직접 확인): 판정 결과는 AI 창작이 아니라 실제 HTTP 응답(헤더 존재 여부·SSL 인증서·경로 상태코드) 기반 결정론적 산출이며, 스캔 1회당 요청 13건(본문 1+경로 프로브 12) 순차 실행·순수 GET만 사용·User-Agent를 `SecurityScanner/1.0`으로 스스로 밝힘 — **승인만 받으면 실제 사이트에 써도 기술적으로 무리 없는 수준**이라고 결론. 다만 **App 17(네트워크 스캐너)과 달리 서버 측 승인 강제(authorized 체크박스+검증)가 없고 UI 문구뿐** — 사용자에게 App 17과 동일한 방식 추가를 제안했으나 아직 진행 여부 결정 안 됨(다음 세션에서 확인 필요)
- **⚠️ 발견한 사소한 이슈(미수정)**: `frontend/src/pages/WebScanner.jsx`의 `SAMPLE_URLS`에 `https://google.com`이 포함돼 있는데, 바로 옆 안내문("본인 소유 또는 허가받은 사이트만 스캔하세요")과 모순되는 예시임 — 로컬 테스트 레인지(Juice Shop) 또는 `testphp.vulnweb.com`으로 교체를 제안했으나 사용자 확인 대기 중
