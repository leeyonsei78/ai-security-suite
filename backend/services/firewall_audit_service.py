...
from services.firewall_audit_offline_engine import analyze_offline
...
def _real_analyze(source_type: str, content: str, context: str, backend: str = "cloud") -> dict:
    ...
    user_prompt = f"방화벽 플랫폼: {label}{context_line}\n\n방화벽 규칙:\n{content}"
    ...
    if backend == "local":
        ...
        usage_log.log_usage("firewall_audit", message.usage, model="claude-sonnet-4-6")
...
async def analyze_firewall(source_type: str, content: str, context: str) -> dict:
