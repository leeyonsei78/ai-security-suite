...
import {
  ShieldQuestion, Terminal, Cloud, CloudCog, Network, Router, Lock, MonitorCog, Server, Trash2, Download,
  AlertTriangle, ListOrdered, BadgeCheck, Upload, FileText, X, WifiOff, FlaskConical,
} from 'lucide-react'
...
const PLACEHOLDERS = {
  ...
  vpn_gateway: `config vpn ssl settings\n set ssl-min-proto-ver tls1-0\n set idle-timeout 0\nend\n\nconfig user local\n edit "vpnuser1"\n set two-factor disable\n next\nend`,
...
const SAMPLE_FILES = {
  ...
  vpn_gateway: '/samples/firewall-audit/vpn-gateway-config.txt',
...
export default function FirewallAudit() {
  ...
  const analyze = async (contentOverride) => {
    ...
    setResult(null)
    try {
      const res = await axios.post('/api/firewall-audit/analyze', { source_type: sourceType, content: body, context })
      setResult(res.data)
          ...
          <div className="md:col-span-2 space-y-4">
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {['iptables', 'aws_sg', 'azure_nsg', 'gcp_fw', 'router_switch', 'vpn_gateway', 'windows_fw', 'other'].map(id => {
