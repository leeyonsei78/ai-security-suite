...
export default function IamAudit() {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file.size > MAX_UPLOAD_BYTES) {
      alert(`파일이 너무 큽니다 (${(file.size / 1024 / 1024).toFixed(1)}MB). 텍스트/JSON 정책 파일은 보통 2MB를 넘지 않습니다.`)
      return
    }
  ...
  const analyze = async () => {
    ...
    setResult(null)
    try {
      const res = await axios.post('/api/iam-audit/analyze', { source_type: sourceType, content, context })
      setResult(res.data)
        ...
        <div className="grid md:grid-cols-5 gap-6">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept=".txt,.json,.log,.csv,text/plain,application/json"
              ...
              />
              <p className="text-[10px] text-slate-600 mt-1">업로드한 파일도 텍스트로 이 입력창에 그대로 채워집니다 — 전송 전 내용을 확인·수정할 수 있습니다. 실제 자격증명 값이 포함된 파일이라면 업로드 전 민감한 값은 마스킹하는 것을 권장합니다.</p>
            </div>
